function run_ppg_beat_detector_assessment
% RUN_PPG_BEAT_DETECTOR_ASSESSMENT  Calls 'ppg_beat_detector_assessment' to
% assess the performance of photoplethysmography (PPG) beat detectors on 
% several datasets, and then collates the results.
%
%  Inputs:  none
%           (NB: the 'ppg_beat_detector_assessment' function, which is called by this function, requires a dataset to analyse)
%
%  Outputs: Plots illustrating the performance of PPG beat detectors
...
if do_analysis
    for dataset_no = 1 : length(up.datasets)
        ...
        ppg_beat_detector_assessment(curr_dataset, options);
...
options.beat_detectors = {'SPAR3', 'SPAR7', 'IMS', 'AMPD', 'MSPTD', 'ABD', 'qppg', 'HeartPy', 'COppg', 'Pulses', 'ERMA', 'PWD', 'PDA', 'WFD'};
